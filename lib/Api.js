...
const refreshModesInterval = 5000;
...
class Api extends Homey.SimpleClass {
    ...
    static ComRingApi = 'api.ring.com';
    static ComRingApp = 'app.ring.com';
    ...
    constructor() {
        ...
        this._ringApiDeferedPromise = defer();
    }
        ...
        Homey.ManagerCloud.getHomeyId((error, cloudId) => {
            ...
            this._verifyAuthentication((error, result) => {
                if (!error)
                {
                    ...
                    systeminformation.uuid = this._adapdUuidToHomeyId.bind(this);
                    ...
                    this._ringApiDeferedPromise.resolve();
                }
            });
        });
    }

    async getRingApi(callback) {
        if (callback) {
            this._ringApiDeferedPromise.promise.then(() => callback(this._ringApi));
        }
        else
        {
            await this._ringApiDeferedPromise.promise;
            return this._ringApi;
        }
    }

    _adapdUuidToHomeyId(callback) {
        this.log('_adapdUuidToHomeyId', this._uniqueid);
        if (callback) {
            callback(this._uniqueid);
        }
        else {
            return new Promise((resolve, reject) => {
                resolve(this._uniqueid);
            });
        }
    }
    ...
    _https (method, host, path, postdata, raw, callback) {
        console.log('_https', method, host, path, postdata);
        ...
        if (method === 'POST' || method === 'PATCH') {
            ...
            console.log('postdata', postdata, this._authenticated);
        ...
        let request = https.request({
            ...
            response.on('end', () => {
                ...
                if (response.statusCode == 204) { //No content
                    ...
                    this._authenticated = false;
                    console.log('_https : ', response.statusCode, response.statusMessage, data);
                    error = new Error(response.statusMessage + ' ' + response.statusCode + ' ' + data);
                } else {
                    try {
                        if (!raw)
                            result = JSON.parse(data);
    ...
    _refreshDevice () {
        ...
        this._https('GET', Api.ComRingApi, '/clients_api/dings/active', null, false, (error, result) => {
            if (error) {
    ...
    getDevices (callback) {
        this._https('GET', Api.ComRingApi, '/clients_api/ring_devices', null, false, (error, result) => {
            callback(error, result);
        });
    }
    ...
    ringChime (device_data, callback) {
        ...
        this._https('POST', Api.ComRingApi, '/clients_api/chimes/' + device_data.id + '/play_sound', null, true, (error, result) => {
            callback(error, result);
        });
    }
    ...
    lightOn (device_data, callback) {
        ...
        this._https('PUT', Api.ComRingApi, '/clients_api/doorbots/' + device_data.id + '/floodlight_light_on', null, true, (error, result) => {
            callback(error, result);
        });
    }
    ...
    lightOff (device_data, callback) {
        ...
        this._https('PUT', Api.ComRingApi, '/clients_api/doorbots/' + device_data.id + '/floodlight_light_off', null, true, (error, result) => {
            callback(error, result);
        });
    }
    ...
    sirenOn (device_data, callback) {
        ...
        this._https('PUT', Api.ComRingApi, '/clients_api/doorbots/' + device_data.id + '/siren_on', null, true, (error, result) => {
            callback(error, result);
        });
    }
    ...
    sirenOff (device_data, callback) {
        ...
        this._https('PUT', Api.ComRingApi, '/clients_api/doorbots/' + device_data.id + '/siren_off', null, true, (error, result) => {
            callback(error, result);
        });
    }
    ...
    async grabImage (device_data, callback) {
        ...
        for (let i = 0; i < retries; i++) {
            this._https('POST', Api.ComRingApi, '/clients_api/snapshots/timestamps', postdata, false, (error, result) => {
                if (error) {
                    this._https_auth_mfa_refresh((error, result) => {
                        if (error) {
                            this._authenticated = false;
        ...
        this._https('GET', Api.ComRingApi, '/clients_api/snapshots/image/' + device_data.id, null, true, (error, result) => {
            if (error) {
    ...
    enableMotion (device_data, callback) {
        ...
        this._https('PATCH', Api.ComRingApi, '/devices/v1/devices/' + device_data.id + '/settings', postdata, false, (error, result) => {
            if (error){
                console.log(error);
            }
            callback(error, result);
    ...
    async enableMode(location_id) {
        var location = await this.getLocation(location_id);
        await location.restClient.request({
            method: 'POST',
            url: 'https://app.ring.com/api/v1/mode/location/' + location_id + '/settings/setup'
        });

        await location.getLocationMode();
    }

    async disableMode(location_id) {
        var location = await this.getLocation(location_id);
    ...
    async setMode(location_id, value) {   
        ...
        var currentMode = await location.getLocationMode();
        console.log('setMode', 'currentMode', currentMode);
        var supportsLocationModeSwitching = await location.supportsLocationModeSwitching();
        console.log('setMode', 'supportsLocationModeSwitching', supportsLocationModeSwitching, 'hasAlarmBaseStation|hasHubs', location.hasAlarmBaseStation, location.hasHubs, location.);
        if (supportsLocationModeSwitching) {
            await location.setLocationMode(value);
        } else {
    ...
    subscribeModeUpdates(location_id, callback) {
        this.getLocation(location_id).then(loc => {
                loc.onLocationMode.subscribe(callback);
        }).catch(error => {
            this.error(error);
        });
    }
