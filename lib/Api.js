...
class Api extends Homey.SimpleClass {
    ...
    async setMode(location_id, value) {   
        var location = await this.getLocation(location_id);
        if (location.supportsLocationModeSwitching()) {
            console.log('setMode', 'supportsLocationModeSwitching true');
            await location.setLocationMode(value);
        } else {
            console.log('setMode', 'supportsLocationModeSwitching false', 'hasAlarmBaseStation', location.hasAlarmBaseStation);
