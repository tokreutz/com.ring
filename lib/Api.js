...
class Api extends Homey.SimpleClass {
    ...
    constructor() {
        ...
        setInterval(this._refreshModes.bind(this), refreshModesInterval);
    ...
    async _refreshModes () {
        console.log('_refreshModes');
        let result = await this.getModes();
        this.emit('refresh_modes', result);
    }
    ...
    async getModes() {
        ...
        return await Promise.all(locations.map(async (loc) => {
            let mode = await this.getMode(loc.location_id);
            return {
                location: loc,
                mode: mode
            };
    ...
    getLocations() {
        return new Promise((resolve, reject) => {
            this._https('GET', Api.ComRingApi, '/devices/v1/locations', null, false, (error, result) => {
                if (error) {
                    console.log(error);
                    reject(error);
                }
                ...
                {
                    let locations = [];
                    result.user_locations.forEach((loc) => {
                        locations.push(loc);
                    });
    ...
    getMode(location_id) {
        return new Promise((resolve, reject) => {
            this._https('GET', Api.ComRingApp, '/api/v1/mode/location/' + location_id, null, false, (error, result) => {
                if (error) {
                    console.log(error);
                    reject(error);
                }
    ...
    enableMode(location_id) {
        return new Promise((resolve, reject) => {
            console.log('enableMode');
    ...
    disableMode(location_id) {
        return new Promise((resolve, reject) => {
            this._https('DELETE', Api.ComRingApp, '/api/v1/mode/location/' + location_id + '/settings', null, false, (error, result) => {
                if (error) {
                    console.log(error);
                    reject(error);
                }
    ...
    setMode(location_id, value) {
        return new Promise((resolve, reject) => {
            let modeData = {
                mode: value
            };

            this._https('POST', Api.ComRingApp, '/api/v1/mode/location/' + location_id, modeData, false, (error, result) => {
                if (error) {
                    console.log(error);
                    reject(error);
                }
